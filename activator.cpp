...
int main () {
	// Custom console 
	system("MODE CON COLS=95 LINES=21");
	
	// Point that can be used if user types the wrong number
	Start:
	system ("cls");
    string passkey="FVYXC2f2";
    bool auth=false;
    string pass;
    /*cout << "██╗    ██╗██╗███╗   ██╗██████╗  ██████╗ ██╗    ██╗███████╗             ";
    cout << "██║    ██║██║████╗  ██║██╔══██╗██╔═══██╗██║    ██║██╔════╝             ";
    ...
    cout << "                                                                       \n\n";*/
	cout << R"(
 _     _  ___   __    _  ______   _______  _     _  _______                    
| | _ | ||   | |  |  | ||      | |       || | _ | ||       |                   
| || || ||   | |   |_| ||  _    ||   _   || || || ||  _____|                   
|       ||   | |       || | |   ||  | |  ||       || |_____                    
|       ||   | |  _    || |_|   ||  |_|  ||       ||_____  |                   
|   _   ||   | | | |   ||       ||       ||   _   | _____| |                   
|__| |__||___| |_|  |__||______| |_______||__| |__||_______|                   
 _______  _______  _______  ___   __   __  _______  _______  _______  ______   
|   _   ||       ||       ||   | |  | |  ||   _   ||       ||       ||    _ |  
|  |_|  ||       ||_     _||   | |  |_|  ||  |_|  ||_     _||   _   ||   | ||  
|       ||       |  |   |  |   | |       ||       |  |   |  |  | |  ||   |_||_ 
|       ||      _|  |   |  |   | |       ||       |  |   |  |  |_|  ||    __  |
|   _   ||     |_   |   |  |   |  |     | |   _   |  |   |  |       ||   |  | |
|__| |__||_______|  |___|  |___|   |___|  |__| |__|  |___|  |_______||___|  |_|                                                                                                                                                                                       
)";
	cout << "Welcome to Windows Activator\n";
    do{
	cout << "Enter Pass Key : ";
    cin >> pass;
    if(passkey==pass){
	...
	system("cls");
	cout << "Acces Granted! \n\n";
	cout << "Welcome to d4x Windows Activator\n";
	...
	cout<<"successfully activated Windows 10!"<< WindowsName <<"! \n";
	cout<<"Thanks for using this! @d4x";*/
}